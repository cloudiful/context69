...
fn registry_is_sorted_unique_and_complete() {
    assert_eq!(
        OPERATIONS.len(),
        116,
        "registry must cover all 116 operations"
    );
    ...
    assert_eq!(unique.len(), 116, "operation ids must be unique");
...
fn registry_matches_openapi_exactly_with_no_omission() {
    ...
    assert_eq!(openapi.len(), 116, "OpenAPI must expose 116 operations");
