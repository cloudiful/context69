...
mod tests {
    ...
    fn password_hash_round_trips() {
        let hash = hash_password("secret-pass").expect("hash password");
        assert!(verify_password(&hash, "secret-pass").is_ok());
        assert!(verify_password(&hash, "wrong-pass").is_err());
    }
    ...
    fn verifies_legacy_argon2_v05_phc_hash() {
        ...
        assert!(verify_password(LEGACY_PHC, "legacy-pass").is_ok());
        assert!(verify_password(LEGACY_PHC, "wrong").is_err());
    }
}
