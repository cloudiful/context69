...
use crate::{
    ...
    services::{
        auth::AuthService, library::LibraryService, query::QueryService,
...
async fn import_legacy_runtime_if_needed(db: &Database, config: &Config) -> Result<()> {
    ...
    if let Some(docling) = &config.docling {
        db.save_docling_settings(&StoredDoclingSettings {
            ...
            pdf_backend: docling.conversion.pdf_backend.clone(),
            images_scale: docling.conversion.images_scale,
            image_export_mode: docling.conversion.image_export_mode.clone(),
            do_ocr: docling.ocr.do_ocr,
            force_ocr: docling.ocr.force_ocr,
            ocr_engine: docling.ocr.ocr_engine.clone(),
            ocr_lang: docling.ocr.ocr_lang.clone(),
            do_code_enrichment: docling.enrichment.do_code_enrichment,
            do_formula_enrichment: docling.enrichment.do_formula_enrichment,
            do_picture_description: docling.enrichment.do_picture_description,
