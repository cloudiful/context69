...
/// Wakeable source-cleanup wiring (issue 389): task-history cleanup keeps
/// its 6-hour cadence untouched, while source releases drain via the shared
/// dispatcher (startup drain, immediate wake after commit, 5-minute
/// fallback). `shutdown` cancels both the history loop and the dispatcher
/// loop for tests and graceful shutdown; production also aborts the task
/// on process exit.
    ...
    tokio::spawn(async move {
        ...
        loop {
            tokio::select! {
                ...
                _ = cycle.tick() => {
                    if let Err(error) = run_cleanup(&history_service).await {
