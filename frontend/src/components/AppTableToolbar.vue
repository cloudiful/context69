...
<template>
  <div class="grid gap-2 rounded-lg border border-(--p-content-border-color)/80 bg-(--p-content-background) px-2 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
    ...
    <div class="flex flex-wrap items-center gap-1.5 max-md:w-full md:justify-end">
      <IconField v-if="searchEnabled" class="relative min-w-0 md:w-72 [&.p-iconfield]:w-full">
        <InputIcon
          v-if="showSearchIcon"
          class="pi pi-search pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-(--p-text-muted-color)"
        />
        <InputText
