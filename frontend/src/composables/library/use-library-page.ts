...
export function useLibraryPage({ folder, t }: Options) {
  ...
  function toEntry(item: LibraryResourceItem, parent: LibraryFolderNode): ExplorerEntry {
    if (item.kind === "folder") {
  ...
  async function loadPage() {
    ...
    error.value = null;
    try {
      const response = await apiClient.getLibraryResources({
