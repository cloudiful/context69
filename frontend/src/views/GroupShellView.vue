...
<template>
  <div class="workspace-page">
    <AppPanel surface="plain" :title="state.group?.name || state.groupKey">
      <template #actions>
        <div class="flex gap-2">
          <Button severity="secondary" variant="outlined" @click="state.groupDialogVisible = true">
            {{ $t("common.edit") }}
          </Button>
          <Button severity="danger" variant="outlined" @click="state.confirmDeleteGroup">
            {{ $t("common.delete") }}
          </Button>
        </div>
      </template>
